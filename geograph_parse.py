...
class GeoMeshGraphParse(object):
    ...
    @property
    def grid(self):
        return self.get_base_grid()

    @property
    def sample_data(self, idx=0):
        if self.input_sample_data is not None:
            return self.input_sample_data
        if len(self.data_sampler.sample_data_list) > 0:
            return self.data_sampler.sample_data_list[idx]
    ...
    def set_geo_sample_data(self, input_sample_data: GeodataSet, grid_dims=None, ext_grid=None, split_ratio=None,
        ...
        geo_borehole_sample.set_pre_val_test_idx(test_idx=[109, 14, 203, 120])
