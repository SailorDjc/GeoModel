...
class ReadExportFile(object):
    ...
    def read_train_loss_log(txt_file_path, **kwargs):
        ...
        if names is None:
            names = ['epoch', 'train_loss', 'save_train_loss', 'train_acc', 'val_loss', 'save_val_loss', 'val_acc']
        ...
        if 'save_val_loss' in df_logs.columns:
            val_loss = np.array(df_logs.loc[:, ['save_val_loss']])
            train_loss = np.array(df_logs.loc[:, ['save_train_loss']])
        result_log = {
