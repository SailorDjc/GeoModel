...
class PointSet(object):
    ...
    def get_convexhull_2d(self, extend_buffer=0) -> np.ndarray:  # 3D points array
        if self.points is None:
            raise ValueError('Need to set points first.')
        ...
        if extend_buffer == 0:
            ...
            poly_points_2d = self.points[line_pnt_idx][:, 0:2]
            convex_polygon = Polygon(poly_points_2d)
            convex_polygon = convex_polygon.buffer(distance=extend_buffer)
            x, y = convex_polygon.exterior.xy
            boundary_area = np.array([x, y]).transpose()
