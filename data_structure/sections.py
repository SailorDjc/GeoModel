...
class Section(object):
    def __init__(self, vtk_data=None, vtk_data_path=None, points: np.ndarray = None, series: np.ndarray = None
                 , sample_spacing=None, sample_grid=None, name=None, dir_path=None, updata_label=False):
        self.name = name
        ...
        if vtk_data is not None or (vtk_data_path is not None and os.path.exists(vtk_data_path)):
            ...
            if isinstance(vtk_data, pv.RectilinearGrid):
                ...
                # 从vtk数据中获取标签
            elif isinstance(vtk_data, (pv.PolyData, pv.UnstructuredGrid)):
                self.dims = None
                self.points = vtk_data.cell_centers().points
                self.points_num = self.points.shape[0]
            else:
                raise ValueError('unsupported vtk data type.')
            self.standardize_labels_from_vtk_data(update_label=updata_label)
    ...
    def standardize_labels_from_vtk_data(self, label_dict: dict = None, default_value=-1, update_label=True):
        ...
        if update_label:
            ...
            self.series = old_label.astype(int)
            self.classes = unique_label.astype(int)
