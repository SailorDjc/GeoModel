...
class GeoDataSampler(object):
    def __init__(self, grid=None, default_value=-1, split_ratio=None):
        ...
        self.val_indexes = []
        self.test_indexes = []
        self.pre_geo_sample_map = None  # {'val': [], 'test': []}

    def set_pre_val_test_idx(self, val_idx=None, test_idx=None):
        if val_idx is None and test_idx is None:
            return
        else:
            self.pre_geo_sample_map = {'val': [], 'test': []}
            if val_idx is not None:
                self.pre_geo_sample_map['val'] = val_idx
            if test_idx is not None:
                self.pre_geo_sample_map['test'] = test_idx
    ...
    def compute_split_data(self, sample_data_num):
        ...
        if self.pre_geo_sample_map is not None:
            val_idx = self.pre_geo_sample_map['val']
            test_idx = self.pre_geo_sample_map['test']
            train_idx = list(set(sample_data_all_idx) - set(val_idx) - set(test_idx))
