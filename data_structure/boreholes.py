...
class BoreholeSet(object):
    ...
    def points_transform(self, modefunc, factor, center=None):
        if center is None:
            center = self.center
        self.points = modefunc(points=self.points, t_factor=factor, center=center)
        for borehole_id in range(len(self.boreholes_list)):
            ...
            self.boreholes_list[borehole_id].points = modefunc(points=self.boreholes_list[borehole_id].points, t_factor=factor, center=center)
    ...
    def generate_vtk_data_as_tube(self, borehole_radius=1.0, is_tube=True):
        ...
        for borehole_id, one_borehole in enumerate(self.boreholes_list):
            ...
            for layer_id, one_layer in enumerate(one_borehole.holelayer_list):
                if is_tube:
                    self.boreholes_list[borehole_id].holelayer_list[layer_id].buffer_dist_xy = borehole_radius
    ...
    def get_boreholes_convexhull_bounding_surface_and_outline(self):
        if self.boreholes_list is None or len(self.boreholes_list) == 0:
