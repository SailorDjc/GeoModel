import numpy as np
from scipy.interpolate import LinearNDInterpolator, splev, splrep
...
# 将一条折线拟合为B样条曲线
def polyline_to_bspline_curve_2d(polyline_points, points_num):
    x = polyline_points[:, 0]
    y = polyline_points[:, 1]
    tck = splrep(x, y)
    x_new = np.linspace(min(x), max(x), points_num)
    y_bspline = splev(x_new, tck)
    bspline_points = np.concatenate((x_new, y_bspline), axis=0)
    return bspline_points


# 根据输入的线, 创建缓冲区，筛选在缓冲区范围内的点，返回沿输入线方向排序的点集合
def select_points_2d_by_line_buffer(points, line_points, buffer_dist):
...
def points_trans_translate(t_factor, points=None, center=None, only_get_matrix=False):
    ...
    for one_point in points:
        trans_point = trans_translate(one_point[0], one_point[1], one_point[2], t_factor[0], t_factor[1], t_factor[2])
...
def add_points_to_point_set_if_no_duplicate(points_set: list, points, tolerance=0.000001, k=3):
    # 判断添加的点集是否有重复
    ckt = spt.cKDTree(points)
    d, pid = ckt.query(points, k=k)
    if len(points_set) == 0:
        pass
...
def extend_bounds(input_bounds, buffer_xy=0, buffer_z=0):
    if len(input_bounds) == 4:
        min_x, max_x, min_y, max_y = input_bounds
        return [min_x - buffer_xy, max_x + buffer_xy, min_y - buffer_xy, max_y + buffer_xy]
    elif len(input_bounds) == 6:
        min_x, max_x, min_y, max_y, min_z, max_z = input_bounds
        return [min_x - buffer_xy, max_x + buffer_xy, min_y - buffer_xy, max_y + buffer_xy
            , min_z - buffer_z, max_z + buffer_z]
    else:
        raise ValueError('bounds input error.')
