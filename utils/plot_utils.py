...
class ColorDict(object):
    ...
    class color_rgb(object):
        ...
        def get_decimal_code(self):
            return self.r * 256 * 256 + self.g * 256 + self.b

    def map_value(self, label):
...
def control_visibility_with_layer_label(geo_object_list: list, lookup_table=None
    ...
    if not isinstance(geo_object_list, (list, GeodataSet)):
        raise ValueError('Input not support.')
    if isinstance(geo_object_list, GeodataSet):
        geo_object_list = [geo_object for geo_object in geo_object_list.geodata_list]
    plotter = pv.Plotter()
...
# 生成混淆矩阵
def plot_matrix(y_true, y_pred, labels_name=None, title=None, threshold=0.8, axis_labels=None, save_path=None):
    ...
    for i in range(np.shape(cm)[0]):
        for j in range(np.shape(cm)[1]):
            if int(cm[i][j] * 100 + 0.5) > 0:
                plt.text(j, i, format(cm[i][j] * 100, '.2f') + '%',  #  + 0.5
                         ha="center", va="center",
    ...
    if save_path is not None:
        ...
        print('{} has been saved to the path {}'.format(pic_name, save_path))
